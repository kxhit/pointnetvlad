...
parser.add_argument('--fold', type=str, default="00", help='1-fold training sequence [default: 00]')
...
#KITTI
KIITI_submap_dir = "/media/work/data/kitti/odometry/submap_seg_bin"
all_sequences = ["00","01","02","03","04","05","06","07","08","09","10"] #1-fold
# all_sequences = ["00","01","02","03","04","05","06","07","08"]
eval_seq = FOLD
TRAINING_FILES = []
TEST_FILES = []
files = []
for sq in all_sequences:
    sq_dir = os.path.join(KIITI_submap_dir, sq)
    if sq != eval_seq:
        listDir(sq_dir, files)
        TRAINING_FILES.extend(files)
        files = []
    else:
        listDir(sq_dir, TEST_FILES)
...
def train_one_epoch(sess, ops, train_writer, test_writer, epoch, saver):
    ...
    train_file_idxs = np.arange(0, len(TRAINING_FILES))
    ...
    for i in range(len(train_file_idxs)//BATCH_NUM_QUERIES):
        ...
        for j in range(BATCH_NUM_QUERIES):
            # if(len(TRAINING_FILES[batch_keys[j]]["positives"])<POSITIVES_PER_QUERY):
            if(len(get_pos(TRAINING_FILES[batch_keys[j]]))<POSITIVES_PER_QUERY):
                faulty_tuple=True
                break
            ...
            if(len(TRAINING_LATENT_VECTORS)==0):
                ...
                q_tuples.append(get_query_tuple(TRAINING_FILES[batch_keys[j]],POSITIVES_PER_QUERY,NEGATIVES_PER_QUERY, TRAINING_FILES, hard_neg=[], other_neg=True))
                ...
                query = get_feature_representation(TRAINING_FILES[batch_keys[j]], sess, ops)
                negtives_list = get_neg(TRAINING_FILES[batch_keys[j]])
                # random.shuffle(TRAINING_QUERIES[batch_keys[j]]['negatives'])
                random.shuffle(negtives_list)
                # negatives=TRAINING_QUERIES[batch_keys[j]]['negatives'][0:sampled_neg]
                negatives = negtives_list[0:sampled_neg]
                hard_negs= get_random_hard_negatives(TRAINING_FILES[batch_keys[j]],query, negatives, num_to_take)
                print(hard_negs)
                q_tuples.append(get_query_tuple(TRAINING_FILES[batch_keys[j]],POSITIVES_PER_QUERY,NEGATIVES_PER_QUERY, TRAINING_FILES, hard_negs, other_neg=True))
        ...
        if(i%200==7):
            ...
            for eval_batch in range(eval_batches):
                ...
                for e_tup in range(BATCH_NUM_QUERIES):
                    ...
                    if (len(get_pos(TEST_FILES[eval_keys[e_tup]])) < POSITIVES_PER_QUERY):
                    ...
                    eval_tuples.append(get_query_tuple(TEST_FILES[eval_keys[e_tup]],POSITIVES_PER_QUERY,NEGATIVES_PER_QUERY, TEST_FILES, hard_neg=[], other_neg=True))
        ...
        # if ( i % 3000 == 0):
            ...
            TRAINING_LATENT_VECTORS = get_latent_vectors(sess, ops, TRAINING_FILES)
...
def get_random_hard_negatives(query_file, query_vec, random_negs, num_to_take):
    global TRAINING_LATENT_VECTORS
    base_path = query_file[:-11]
    latent_vecs=[]
    for j in range(len(random_negs)):
        random_neg = os.path.join(base_path, '%06d' % int(random_negs[j]) + ".bin")
        index = np.array(np.argwhere(np.array(TRAINING_FILES)==random_neg)).reshape(-1)
        # print(random_negs[j])
        # print(random_neg)
        # latent_vecs.append(TRAINING_LATENT_VECTORS[random_negs[j]])
        latent_vecs.append(TRAINING_LATENT_VECTORS[index])

    latent_vecs=np.array(latent_vecs).squeeze(axis=1)
...
def get_latent_vectors(sess, ops, files_to_process):
    is_training=False
    # train_file_idxs = np.arange(0, len(dict_to_process.keys()))
    train_file_idxs = np.arange(0, len(files_to_process))
    ...
    for q_index in range(len(train_file_idxs)//batch_num):
        ...
        for index in file_indices:
            ...
            file_names.append(files_to_process[index])
    ...
    #handle edge case
    for q_index in range((len(train_file_idxs)//batch_num*batch_num),len(files_to_process)):
        index=train_file_idxs[q_index]
        # queries=load_pc_files([dict_to_process[index]["query"]])
        queries = load_pc_files([files_to_process[index]])
        queries= np.expand_dims(queries,axis=1)
